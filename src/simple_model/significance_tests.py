...
def load_clean_data() -> pd.DataFrame:
    """
    Load the preprocessed dataset from preprocessing.
    Required columns:
        - team_prob: bookmaker implied probability for tracked team
        - opp_prob: opponent implied probability
        - win: 1 if team won, 0 if lost
    """
...
def train_logistic_model(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Train logistic regression model and return:
...
# Paired tests: Brier Score + Log-loss


def paired_logloss_test(book_prob: np.ndarray, model_prob: np.ndarray, outcomes: np.ndarray
                        ) -> Tuple[float, float, float, float, float]:
    """
    Paired t-test on per-bet log-loss.
    Returns:
        bookmaker LL, model LL, diff, t-stat, p-value
    """
    eps = 1e-15
...
def paired_brier_test(book_prob: np.ndarray, model_prob: np.ndarray, outcomes: np.ndarray
                      ) -> Tuple[float, float, float, float, float]:
    """
...
def run_significance_tests() -> None:
    ...
    ll_book, ll_model, ll_diff, ll_t, ll_p = paired_logloss_test(book_prob, model_prob, y_test)
